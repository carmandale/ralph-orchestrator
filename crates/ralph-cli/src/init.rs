...
# Generated by: ralph-o init --backend {backend}
...
# Create PROMPT.md with your task, then run: ralph-o run
"#
    )
}
...
fn check_file_exists(force: bool) -> Result<(), InitError> {
    ...
    if agent_dir.exists() && agent_dir.is_dir() {
        ...
        eprintln!("   Run `ralph-o run` to automatically migrate to the new session-based structure.");
...
pub fn format_preset_list() -> String {
    ...
    output.push_str("\nUsage: ralph-o init --preset <preset-name>\n");
    output
}
